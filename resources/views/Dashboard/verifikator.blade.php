...
<div class="row g-3 mb-3" id="summaryCards">
    @foreach($summaryDefs as $sd)
        <div class="col-6 col-sm-3">
            <div class="card border-0 shadow-sm h-100 verif-summary-card"
                 role="button"
                 data-key="{{ $sd['key'] }}"
...
(function () {
    ...
    // ── DATA ──────────────────────────────────────────────
    const allProjects  = @json($allProjectsData);
    ...
    function updateAntrian(key) {
        ...
        document.getElementById('antrianTitle').innerHTML =
            `<i class="bi bi-list-check me-1"></i>${key ? antrianTitles[key] : 'Detail Summary Card'}`;
        document.getElementById('antrianSubtitle').textContent =
            key ? antrianSubtitles[key] : 'Klik summary card untuk lihat daftar';
    ...
    function updateMatrix(filtered) {
        document.querySelectorAll('.matrix-cell').forEach(cell => {
            const { dept, status } = cell.dataset;
        ...
        // row totals per dept
        document.querySelectorAll('.matrix-row-total').forEach(cell => {
            const count = filtered.filter(p => p.dept === cell.dataset.dept).length;
            cell.textContent = count > 0 ? count : '—';
        });

        // column totals per status
        document.querySelectorAll('.matrix-status-total').forEach(cell => {
            const count = filtered.filter(p => p.status === cell.dataset.status).length;
            cell.textContent = count > 0 ? count : '—';
        });

        // grand total
        const grandEl = document.getElementById('matrixGrandTotal');
        if (grandEl) grandEl.textContent = filtered.length > 0 ? filtered.length : '—';
    ...
    document.querySelector('#matrixBody').addEventListener('click', function (e) {
        ...
        document.getElementById('matrixDetailTitle').innerHTML = `
        ...
        document.getElementById('matrixDetailList').innerHTML = projects.map(p => `
        ...
        document.getElementById('matrixDetail').style.display = '';
        ...
        new Chart(areaCanvas, {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    data: @json(collect($chartData)->pluck('jumlah')),
                    fill: true,
